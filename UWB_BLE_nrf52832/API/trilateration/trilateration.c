...
//    vec3d   solution_compare1;
//    vec3d   solution_compare2;
...
void Th_Location(vec3d * anchorArray,int * distanceArray)
{
    ...
    struct point_t points[1];
//    struct point_t Location[1];
...
{
    static JustForSend_t send_x, send_y;
    
    //unit vector in a direction from point1 to point 2
    double p2p1Distance = pow(pow(point2.x-point1.x,2) + pow(point2.y-   point1.y,2),0.5);  // 1 2 两点之间的 距离 
    ...
    //signed magnitude of the x component 有符号的x分量的大小
    double i = ex.x * aux.x + ex.y * aux.y;
    //the unit vector in the y direction. y方向上的单位向量。
    ...
    MsgToMaster[9]  = send_x.i >> 8;
    MsgToMaster[10] = send_x.i & 0xFF;
    
    MsgToMaster[11] = send_y.i >> 8;
    MsgToMaster[12] = send_y.i & 0xFF;
...
void Th_Location2(vec3d * anchorArray, int * distanceArray)
{
    ...
    point1.y = anchorArray[0].y;    // 只用来计算2维坐标 
    ...
    r1 = (double)distanceArray[0]/100;
    r2 = (double)distanceArray[1]/100;
    r3 = (double)distanceArray[2]/100;
